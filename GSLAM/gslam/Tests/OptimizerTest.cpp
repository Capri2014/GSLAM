...
TEST(Optimizer,OptimizePoseSimulate)
{
    for(int It=0;It<1;It++)
    {
        ...
        groundPose=GSLAM::SE3(pi::SO3d::exp(Point3d(0,0,0.3)),pi::Point3d(1,0,0));
        for(int i=0;i<N;i++)
        {
        ...
        SPtr<Optimizer> opt=GSLAM::Optimizer::create();
        opt->_config.verbose=true;
        EXPECT_TRUE(opt->optimizePose(estMatches,estIDepth,estPose));

        std::cout<<"GroundPose:"<<groundPose<<",EstPose:"<<estPose<<std::endl;
    }
}
...
TEST(Optimizer,OptimizePnPSimulate)
{
    ...
    std::cout<<"GroundPose:"<<groundPose<<",EstPose:"<<estPose<<",Time:"<<ticToc.Tac()*1000<<"ms"<<std::endl;
}
...
TEST(Optimizer,SE3GraphSimulate)
{
     ...
     double noise=svar.GetDouble("SE3GraphSimulate.Noise",2e-3);
     ...
     for(size_t i=0;i<N;i++)
     {
         ...
         graph.se3Graph.push_back({i,i+1,noised_odo,NULL});
         if(i==0)
...
TEST(Optimizer,SIM3GraphSimulate)
{
    ...
    double noise=svar.GetDouble("SIM3GraphSimulate.Noise",2e-3);
    GSLAM::BundleGraph graph;
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        graph.keyframes.push_back(currentFrame);
        graph.sim3Graph.push_back({i,i+1,noised_odo,NULL});
        if(i==0)
            graph.keyframes.back().dof=UPDATE_KF_NONE;// fixed
        else if(i+1==N)
...
TEST(Optimizer,GPSGraphSimulate)
{
    ...
    double noise=svar.GetDouble("GPSGraphSimulate.OdoNoise",2e-3);
    ...
    for(size_t i=0;i<N;i++)
    {
        GSLAM::SIM3 noised_odo=odometer*GSLAM::SIM3(SO3::exp(Point3d(Random::RandomGaussianValue(0.,noise),
                                                  ...
                                                  Point3d(Random::RandomGaussianValue(0.,noise),
                                                          ...
                                                          Random::RandomGaussianValue(0.,noise)),
                                                   Random::RandomGaussianValue(1.,noise));
        GSLAM::SE3 noised_gps=curGPS.get_se3()*GSLAM::SE3(SO3::exp(Point3d(Random::RandomGaussianValue(0.,gpsNoise),
                                       ...
                                       Point3d(Random::RandomGaussianValue(0.,gpsNoise),
                                               ...
                                               Random::RandomGaussianValue(0.,gpsNoise)));
        graph.keyframes.push_back(currentFrame);
        graph.gpsGraph.push_back({i,noised_gps,NULL});
